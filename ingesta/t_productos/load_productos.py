...
LOG_FILE_PATH = "./logs/load_productos.log"
...
def ingest():
    ...
    file_path = os.path.join(BASE_DIRECTORY, "productos.json")
    if not os.path.isfile(file_path):
        logger.warning(f"No se encontró el archivo 'productos.json' en '{BASE_DIRECTORY}'. Nada para subir.")
        return

    # Subir archivo al bucket S3
    s3_file_path = "productos/productos.json"  # Ruta en el bucket S3
    try:
