...
def ingest():
    ...
    file_path = os.path.join(BASE_DIRECTORY, "comentarios.json")
    if not os.path.isfile(file_path):
        logger.warning(f"No se encontró el archivo 'comentarios.json' en '{BASE_DIRECTORY}'. Nada para subir.")
        return

    # Subir archivo al bucket S3
    s3_file_path = "comentarios/comentarios.json"  # Ruta en el bucket S3
    try:
